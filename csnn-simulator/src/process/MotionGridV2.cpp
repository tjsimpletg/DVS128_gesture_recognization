...
//	MotionGridV2
//
static RegisterClassParameter<MotionGridV2, ProcessFactory> _register_1("MotionGridV2");

MotionGridV2::MotionGridV2() : UniquePassProcess(_register_1), _expName(""), _draw(0), _frames_width(0), _frames_height(0),
...
MotionGridV2::MotionGridV2(std::string expName, size_t draw, size_t frames_width, size_t frames_height, size_t frames_total, size_t mg_vertical_frames, size_t mg_horizontal_frames, size_t scaler) : MotionGridV2()
{
...
void MotionGridV2::process_train(const std::string &label, Tensor<float> &sample)
{
	_process(label, sample);
}

void MotionGridV2::process_test(const std::string &label, Tensor<float> &sample)
{
	_process(label, sample);
}

Shape MotionGridV2::compute_shape(const Shape &shape)
{
	_height = shape.dim(0);
	_width = shape.dim(1);
	_depth = 1;
...
// void MotionGridV2::_process(const std::string &label, Tensor<InputType> &in) const
// {
// 	///////////////////////////////
...
// 				float a = _scaler * _frame_diff.at<float>(y, x);
// 				float b = _scaler * _frame_diff.at<float>(y, x);
...
// 			for (int x = 0; x < _frame_size.width; x++)
// 			{
// 				// float a = _scaler * (std::abs(_frame_diff.at<float>(y, x)) - _frame_diff.at<float>(y, x)) / 2;
// 				// float b = _scaler * (std::abs(_frame_diff_2.at<float>(y, x)) - _frame_diff_2.at<float>(y, x)) / 2;
...
// 				// else
// 				// {
// 				// 	Vxminus.at<uint8_t>(y, x) = (uint8_t)(-a);
// 				// 	Vxplus.at<uint8_t>(y, x) = 0;
// 				// }
...
// 				// else
// 				// {
// 				// 	Vyminus.at<uint8_t>(y, x) = (uint8_t)(-b);
// 				// 	Vyplus.at<uint8_t>(y, x) = 0;
// 				// }

// 				float a = _frame_diff.at<float>(y, x); //_scaler * (std::abs(flow_parts[0].at<float>(y, x)) - flow_parts[0].at<float>(y, x)) / 2;
// 				float b = _frame_diff_2.at<float>(y, x); //_scaler * (std::abs(flow_parts[1].at<float>(y, x)) - flow_parts[1].at<float>(y, x)) / 2;
