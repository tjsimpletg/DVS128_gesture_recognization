...
EmptyExecution::EmptyExecution(ExperimentType &experiment) : _experiment(experiment), _train_set(), _test_set()
{
	_file_path = std::filesystem::current_path();
}

EmptyExecution::EmptyExecution(ExperimentType &experiment, bool save_features, bool draw_features) : _experiment(experiment), _save_features(save_features), _draw_features(draw_features), _train_set(), _test_set()
{
	_file_path = std::filesystem::current_path();
}

void EmptyExecution::process(size_t refresh_interval)
{
	_load_data();
...
Tensor<Time> EmptyExecution::compute_time_at(size_t i) const
{
	throw std::runtime_error("Unimplemented");
}

void EmptyExecution::_load_data()
{
...
void EmptyExecution::_process_output(size_t index)
{
	for (size_t i = 0; i < _experiment.output_count(); i++)
	{
		if (_experiment.output_at(i).index() == index)
		{
			...
			{
				std::filesystem::create_directories(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/train/");
				std::filesystem::create_directories(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/test/");
			}
			if (_save_features)
			{
				SavePairVector(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/train/" + _experiment.name() + ".json", output_train_set);
				SavePairVector(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/test/" + _experiment.name() + ".json", output_test_set);
			}
			if (_draw_features)
			{
				DrawSparseFeatures(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/train/" + _experiment.name() + "_", output_train_set);
				DrawSparseFeatures(_file_path + "/ExtractedFeatures/" + _experiment.name() + "/test/" + _experiment.name() + "_", output_test_set);
			}
