...
AutoFrameNumberSelector::AutoFrameNumberSelector(const std::string &video_folder_path, size_t &frame_per_video) : _video_folder_path(video_folder_path)
{
	...
	if (frame_per_video == 0)
	{
		...
		for (const auto &file : std::filesystem::directory_iterator(_video_folder_path))
		{
			...
			if (_file_path.find("Weizmann") != std::string::npos)
				for (const auto &_subject_file : std::filesystem::directory_iterator(_file_path))
					for (const auto &_action_file : std::filesystem::directory_iterator(_subject_file))
...
std::string AutoFrameNumberSelector::to_string() const
{
