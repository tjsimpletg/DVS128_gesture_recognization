#ifndef _PROCESS_MOTION_GRID_V4_H
#define _PROCESS_MOTION_GRID_V4_H
...
/**
 * 	MotionGridV4 - GO CHECK PYTHON CODE IN /src/process/OpticalFlowProcess IN ORDER TO GENERATE MG
 */
namespace process
{

	/**
	 * @brief MotionGridV4 takes two frame differences from each three consiqutive frames and seperates them into +ve and -va frames (where the -ve is taken as absolute value). Then these values are put into a motion grids.
	 ...
	 *
	 */
	class MotionGridV4 : public UniquePassProcess
	{
	public:
		MotionGridV4();
		MotionGridV4(std::string expName, size_t draw = 0, size_t frames_width = 0, size_t frames_height = 0, size_t frames_total = 48, size_t mg_vertical_frames = 8, size_t mg_horizontal_frames = 16, size_t scalar = 50);
