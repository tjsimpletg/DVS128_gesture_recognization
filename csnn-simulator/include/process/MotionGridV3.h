#ifndef _PROCESS_MOTION_GRID_V3_H
#define _PROCESS_MOTION_GRID_V3_H
...
/**
 * 	MotionGridV3 - GO CHECK PYTHON CODE IN /src/process/OpticalFlowProcess IN ORDER TO GENERATE MG
 */
namespace process
{

	/**
	 * @brief MotionGridV3 takes the frame difference of two consiqutive frames and seperates them into +ve and -va frames (where the -ve is taken as absolute value). Then these values are put into a grid into a motion grids.
	 ...
	 *
	 */
	class MotionGridV3 : public UniquePassProcess
	{
	public:
		MotionGridV3();
		MotionGridV3(std::string expName, size_t draw = 0, size_t frames_width = 0, size_t frames_height = 0, size_t frames_total = 24, size_t mg_vertical_frames = 4, size_t mg_horizontal_frames = 8, size_t scalar = 50);
