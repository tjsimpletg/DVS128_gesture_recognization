#ifndef _DATASET_VIDEO_H
#define _DATASET_VIDEO_H
...
/**
 * @brief An image sample in VIDEO is 28x28 and with a depth of 1 correspoiding to Greyscale.
 *
 * @param VIDEO_WIDTH is the width of the frame.
 * @param VIDEO_HEIGHT is the height of the frame.
 * @param VIDEO_DEPTH is the number of channels.
 * @param CONV_DEPTH is the depth of the convolution in case of 3D convolution.
 */
#define VIDEO_WIDTH 1
#define VIDEO_HEIGHT 1
#define VIDEO_DEPTH 1
#define CONV_DEPTH 1
...
namespace dataset
{
	 ...
	 * @param threshold video frames that that contain moving pixels with a sum that is less than this threshould are discarded.
	 * @param sample_per_video allows taking multiple samples from each video, is zero, only one sample is taken from a video.
	 * @param grey_video if 0 the video is RGB, if 1 the video is transformed into greyscale.
	 * @param exp_name The name of the expirement.
	 * @param draw A flag that allows drawing the input samples as frames in the Input_frames folder in the build folder.
	...
	public:
		Video(const std::string &video_folder_name, const size_t &frame_per_video, const size_t &frame_gap = 0, const size_t &threshold = 0,
			  const size_t &sample_per_video = 0, const size_t &grey_video = 0,
			  std::string exp_name = "", const size_t &draw = 0, const size_t &frame_size_width = 0, const size_t &frame_size_height = 0, size_t max_read = std::numeric_limits<size_t>::max());
		/**
		 ...
		 * @param frame_gap
		 */
		// virtual void set_frame_gap(cv::VideoCapture capture, cv::Mat skipFrame);
